...
public class BookHandler {
     ...
     * 根据状态查询图书
     *
     * @param state 状态代码 (需要转换类型)
     * @return 查询结果 (只返回前十条数据)
     */
    @GetMapping("/selectBookByState/{state}")
    @ResponseBody
    public List<BookStateVO> selectBookByState(@PathVariable String state){
    ...
    @ResponseBody
    public BookPageDisplayVo selectBookByKeyWord(@RequestBody BookFuzzySearchVO condition){
     ...
     * @return 图书详细信息
     */
    @GetMapping("/selectBookById/{id}")
    @ResponseBody
    public BookDetailVO selectBookById(@PathVariable String id){
    ...
    @ResponseBody
    public List<RecommendBookVO> selectRecommendBook(@PathVariable String bookId){
